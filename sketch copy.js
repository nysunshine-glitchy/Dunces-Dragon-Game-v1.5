...
bullets = new Group();
  ...
  bullets.image = arrowimg;
   bullets.rotation = () => degrees(atan2(mouseY - player.y, mouseX - player.x)); // point toward mouse
  bullets.direction = () => degrees(atan2(mouseY - player.y, mouseX - player.x)); // shoot toward mouse
  bullets.speed = 10;
  bullets.life = 200;
  bullets.w = 20;
  bullets.h = 5;
...
function debugHUD() {
  push();
  noStroke(); fill(0,150); rect(5,5,250,90,6);
  fill(255); textSize(12);
  text(
    `state: ${state}
goblins: ${goblins ? goblins.length : 0}
bullets: ${bullets ? bullets.length : 0}
floorbg: ${floorbg ? floorbg.width+'x'+floorbg.height : 'MISSING'}`,
    12, 24
  );
  pop();
}
...
player.rotation = degrees(atan2(mouseY - player.y, mouseX - player.x));
...
if (mouse.presses()) {
  cursor('crosshair');
  shootsound.play();
  bullets.amount++; // spawns one using the group’s current properties (direction toward mouse)
} else {
  cursor('crosshair');
}

  newGoblinCounter++;
  if (newGoblinCounter > 100) {
    newGoblinCounter = 0;
    let newGob;
     do {
    if (newGob) newGob.remove();
  ...
  for (let g of goblins) {
    ...
    for (let b of bullets) b.direction = b.rotation;
...
function victory() {
  ...
  if (state == "victory" && kb.presses("ESC")) {
    state = "title";
    goodsound.play();
    player.health = 10;
    goblins.removeAll();
